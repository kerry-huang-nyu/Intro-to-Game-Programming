...
void LoadScreen::initialise()
{
    ...
    m_state.map = new Map(LEVEL_WIDTH, LEVEL_HEIGHT, LEVEL_DATA_LOAD, map_texture_id, 1.0f, 24, 8);  // Look at this beautiful initialisation. That's literally it
    ...
    m_state.player->m_height = 2.0f;
    m_state.player->m_width = 2.0f;
...
void LoadScreen::update(float delta_time)
{
    m_state.player->update(delta_time, m_state.player, m_state.enemies, m_state.map);
}

void LoadScreen::render(ShaderProgram* program)
{
