...
void Level3::initialise()
{
    ...
    //We need the Goals3 
    Goals3.have = { 0, 0, 0, 3, 2, 1 };
    Goals3.forbid = { true, false, true, false, false, false };
...
void Level3::spawn(float x, float y, int fruitindex)
{
...
void Level3::spawn_new_fruits(std::vector<Info>& info) {
    for (Info& fruit : info) {
        spawn(fruit.x, fruit.y, fruit.index);
    }
}

bool Level3::check_win()
{
    ...
    for (int i = 0; i < 6; i++) {
        if (Goals3.have[i] > count[i]) {
            m_state.endgame = false;
            break;
        }
        if ((Goals3.forbid[i] == true) && (count[i] > 0)) {
            m_state.endgame = false;
            break;
        }
    }
...
void Level3::update(float delta_time, std::ofstream& log)
{
    if (check_win() == true) {
        return;
    }
...
void Level3::render_next_fruit(ShaderProgram* program, ShaderProgram* text_program, std::ofstream& log)
{
...
void Level3::render_Goals3(ShaderProgram* program, ShaderProgram* text_program, std::ofstream& log) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (Goals3.forbid[i] == true) { //if they have no criterias 
            message = names[i] + " prohibited";
        }
        else if (Goals3.have[i] != 0) {
            message = names[i] + " >= " + std::to_string(Goals3.have[i]);
        }
        else {
...
void Level3::render(ShaderProgram* program, ShaderProgram* text_program, std::ofstream& log)
{
    ...
    render_Goals3(program, text_program, log);
